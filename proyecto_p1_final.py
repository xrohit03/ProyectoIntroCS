"""
CS1000 - Introducción a las Ciencias de la Computación
Proyecto 1 (P1) - 2026-1
Profesor: Ian Paul Brossard
"""

import pandas as pd
import numpy as np
import re
import warnings
warnings.filterwarnings('ignore')

from sklearn.feature_extraction.text import TfidfVectorizer, ENGLISH_STOP_WORDS
from sklearn.cluster import KMeans
from sklearn.preprocessing import normalize

# ============================================================
# CARGA DEL DATASET
# ============================================================

data_frame = pd.read_csv('smogon.csv')
data_frame.columns = data_frame.columns.str.strip().str.lower()

# Remplazamos del Data frame los valores vacíos(NaN) por texto vacío.
data_frame['moves'] = data_frame['moves'].fillna('')

print(f"Dataset cargado: {data_frame.shape[0]} Pokémon, {data_frame.shape[1]} columnas")
print(f"Columnas: {data_frame.columns.tolist()}")
print(f"Valores nulos en 'moves': {data_frame['moves'].isna().sum()}")

# ============================================================
# LOS 18 TIPOS (y los 17 sin normal para verificación)
# ============================================================
TIPOS_POKEMON = [
    'normal', 'fire', 'water', 'electric', 'grass', 'ice',
    'fighting', 'poison', 'ground', 'flying', 'psychic', 'bug',
    'rock', 'ghost', 'dragon', 'dark', 'steel', 'fairy'
]

# Para la verificación excluimos 'normal' porque aparece en TODOS los
# Pokémon (casi todos los moves genéricos son tipo Normal) y no
# aporta información para distinguir grupos.
TIPOS_SIN_NORMAL = [t for t in TIPOS_POKEMON if t != 'normal']

# ============================================================
# STOP WORDS PERSONALIZADAS
# ============================================================

# Partimos de la lista estándar de stop words en inglés
my_stop_words = list(ENGLISH_STOP_WORDS)

# "fire" está en los stop words por defecto, lo eliminamos
# para que el vectorizador sí lo tome en cuenta
my_stop_words.remove("fire")

# Agregamos palabras que no aportan al agrupamiento
my_stop_words.append("pp")        # "pp" no estará dentro del vocabulario
my_stop_words.append("power")     # "power" no estará dentro del vocabulario
my_stop_words.append("accuracy")  # "accuracy" no estará dentro del vocabulario

# ============================================================
# FUNCIÓN DE PREPROCESAMIENTO
# ============================================================
def preprocesar_moves(texto, tipos):
    """
    Separa cada tipo de su contexto pegado usando dos estrategias:
    1. Reemplaza \\xa0 (espacio no rompible) por espacio normal.
    2. Inserta espacio ANTES de aplicar .lower(), buscando el tipo
       con la primera letra en mayúscula (como aparece en el CSV:
       'AttractNormal', 'Body SlamFire', etc.)
    """
    texto = str(texto)
    texto = texto.replace('\xa0', ' ')          # espacio no rompible → normal
    for tipo in tipos:
        # Busca "Fire", "Water", "Normal"... y les pone espacio
        texto = re.sub(tipo.capitalize(), f' {tipo.capitalize()} ', texto)
    texto = texto.lower()                        # ahora sí a minúsculas
    return texto


# ============================================================
# PREGUNTA 1: TF-IDF CON N-GRAMAS
# ============================================================
print("\n" + "=" * 70)
print("PREGUNTA 1: AGRUPAMIENTO MEDIANTE TF-IDF CON N-GRAMAS")
print("=" * 70)

print("\n[1.1] Generando matriz TF-IDF con unigramas y bigramas...")

# Usamos unigramas + bigramas para capturar frases como "fire punch", "water gun", etc.
tf_idf_vectorizer = TfidfVectorizer(
    ngram_range=(1, 2),             # se acepta unigramas y bigramas
    stop_words=my_stop_words,       # palabras a no tomar en cuenta
    max_features=5000,              # limitamos vocabulario
    min_df=2,                       # el n-grama debe aparecer en al menos 2 pokemones
    sublinear_tf=True,              # reducir la diferencia entre las frecuencias con log(tf)
    token_pattern=r'[a-zA-Z]{3,}'  # solo tokens alfabéticos de al menos 3 letras
)

# Definimos variables
ataques = data_frame['moves'].tolist()
matriz_tf_idf = tf_idf_vectorizer.fit_transform(ataques)
vocabulario = tf_idf_vectorizer.get_feature_names_out()

# ============================================================
# [1.2] Mostrar el número total de columnas
# ============================================================
print(f"\n[1.2] Número total de columnas (elementos del vocabulario): {len(vocabulario)}")

# ============================================================
# [1.3] Imprimir los elementos del vocabulario
# ============================================================
print(f"\n[1.3] Elementos del vocabulario:")
for i, elemento in enumerate(vocabulario):
    print(f"  {i+1:3d}. {elemento}")
print(f"  ... (total: {len(vocabulario)} elementos)")

# ============================================================
# [1.4] Generar un DataFrame con la matriz tf-idf que tenga como
# cabeceras los elementos de su vocabulario. Imprimir dicha matriz.
# ============================================================
print("\n[1.4] Generando DataFrame con la matriz TF-IDF...")
data_frame_matriz_tf_idf = pd.DataFrame(
    matriz_tf_idf.toarray(),
    columns=vocabulario,
    index=data_frame['pokemon']
)
# Imprimimos el DataFrame realizado con la matriz TF-IDF
print("Imprimiendo matriz TF-IDF (print):")
print(data_frame_matriz_tf_idf)
print(f"\nDimensiones del DataFrame TF-IDF: {data_frame_matriz_tf_idf.shape}")

# ============================================================
# [1.5] Clustering con K-Means
# ============================================================
print(f"\n[1.5] Aplicando K-Means con 18 clusters...")

# Normalizamos para mejor clustering con TF-IDF
tf_idf_normalizado = normalize(matriz_tf_idf)

# Creamos el objeto KMeans
k_means = KMeans(
    n_clusters=18,
    random_state=42,
    n_init=20,
    max_iter=500
)

# Aplica K-Means a la matriz TF-IDF normalizada
k_means.fit(tf_idf_normalizado)

# Obtiene el cluster de cada Pokémon y lo guardamos en el DataFrame
labels = k_means.labels_
data_frame['cluster_p1'] = labels

# ============================================================
# [1.6] Guardamos el CSV con Pokémon y cluster
# ============================================================
resultado = data_frame[['pokemon', 'cluster_p1']].copy()
resultado.to_csv('resultado_pregunta1.csv', index=False)
print("\n[1.6] Archivo 'resultado_pregunta1.csv' guardado.")

# ============================================================
# [1.7] Mostramos el contenido de cada cluster
# ============================================================
print("\n[1.7] Contenido de cada cluster (Pregunta 1):")
for cluster in range(18):
    pokemon_cluster = data_frame[data_frame['cluster_p1'] == cluster]['pokemon'].tolist()
    print(f"\n  Cluster {cluster} ({len(pokemon_cluster)} Pokémon):")
    print(f"  {', '.join(pokemon_cluster[:20])}", end='')
    if len(pokemon_cluster) > 20:
        print(f" ... (+{len(pokemon_cluster)-20} más)")
    else:
        print()

# Mostramos 10 términos más relevantes de cada cluster
print("\n[1.7b] Top 10 términos TF-IDF más relevantes por cluster (P1):")
order_centroids = k_means.cluster_centers_.argsort()[:, ::-1]
terms = tf_idf_vectorizer.get_feature_names_out()

for cluster in range(18):
    top_terms = [terms[ind] for ind in order_centroids[cluster, :10]]
    print(f"  Cluster {cluster}: {', '.join(top_terms)}")


# ============================================================
# PREGUNTA 2: VOCABULARIO CONTROLADO
# ============================================================
print("\n" + "=" * 70)
print("PREGUNTA 2: AGRUPAMIENTO CON VOCABULARIO CONTROLADO (18 TIPOS)")
print("=" * 70)

print("\n[2.1] Preprocesando texto...")
data_frame['moves_procesado'] = data_frame['moves'].apply(
    lambda x: preprocesar_moves(x, TIPOS_POKEMON)
)

# Ejemplo de preprocesamiento
# Usamos str.lower() porque .str.lower() convirtió todos los nombres a minúsculas
print("\n  Ejemplo (Arcanine, primeros 300 chars):")
print(f"  ORIGINAL:  {data_frame[data_frame['pokemon'].str.lower()=='arcanine']['moves'].iloc[0][:300]}")
print(f"  PROCESADO: {data_frame[data_frame['pokemon'].str.lower()=='arcanine']['moves_procesado'].iloc[0][:300]}")

print("\n[2.2] Generando TF-IDF con vocabulario controlado (18 tipos)...")
tfidf_vocab_controlado = TfidfVectorizer(
    vocabulary=TIPOS_POKEMON,
    token_pattern=r'[a-zA-Z]+'
)
matriz_tf_idf_p2 = tfidf_vocab_controlado.fit_transform(
    data_frame['moves_procesado'].tolist()
)

data_frame_matriz_tf_idf_p2 = pd.DataFrame(
    matriz_tf_idf_p2.toarray(),
    columns=TIPOS_POKEMON,
    index=data_frame['pokemon']
)
print(f"\n  Dimensiones matriz TF-IDF vocabulario controlado: {data_frame_matriz_tf_idf_p2.shape}")
print("\n  Primeras 10 filas:")
print(data_frame_matriz_tf_idf_p2.head(10))

print(f"\n[2.4] Aplicando K-Means con 18 clusters...")
tf_idf_normalizado_p2 = normalize(matriz_tf_idf_p2)
k_means_p2 = KMeans(
    n_clusters=18,
    random_state=42,
    n_init=50,
    max_iter=500
)
k_means_p2.fit(tf_idf_normalizado_p2)

labels_p2 = k_means_p2.labels_
data_frame['cluster_p2'] = labels_p2

resultado_p2 = data_frame[['pokemon', 'cluster_p2']].copy()
resultado_p2.to_csv('resultado_pregunta2.csv', index=False)
print("\n[2.5] Archivo 'resultado_pregunta2.csv' guardado.")

print("\n[2.6] Clusters (Pregunta 2) con tipos dominantes:")
for cluster in range(18):
    idx = data_frame[data_frame['cluster_p2'] == cluster].index
    pokemon_cluster = data_frame[data_frame['cluster_p2'] == cluster]['pokemon'].tolist()
    puntajes = data_frame_matriz_tf_idf_p2.iloc[idx].mean()
    top_tipos = puntajes[puntajes > 0].sort_values(ascending=False).head(3)
    tipos_str = ', '.join([f"{t}={v:.3f}" for t, v in top_tipos.items()])
    print(f"\n  Cluster {cluster} ({len(pokemon_cluster)} Pokémon) → Tipos: {tipos_str}")
    print(f"  Pokémon: {', '.join(pokemon_cluster[:15])}", end='')
    if len(pokemon_cluster) > 15:
        print(f" ... (+{len(pokemon_cluster)-15} más)")
    else:
        print()


# ============================================================
# PREGUNTA 3: VERIFICACIÓN
# ============================================================
print("\n" + "=" * 70)
print("PREGUNTA 3: VERIFICACIÓN DE AGRUPAMIENTO POR TIPOS")
print("=" * 70)

print("\n[3.1] Contando ocurrencias de cada tipo (excluyendo 'normal')...")
for tipo in TIPOS_POKEMON:
    data_frame[f'count_{tipo}'] = data_frame['moves_procesado'].apply(
        lambda x: len(re.findall(r'\b' + tipo + r'\b', str(x)))
    )

def tipo_dominante(row, tipos):
    conteos = {t: row[f'count_{t}'] for t in tipos}
    max_val = max(conteos.values())
    if max_val == 0:
        return 'none'
    return max(conteos, key=lambda t: conteos[t])

def segundo_tipo_dominante(row, tipos):
    conteos = {t: row[f'count_{t}'] for t in tipos}
    sorted_tipos = sorted(conteos.items(), key=lambda x: x[1], reverse=True)
    if len(sorted_tipos) < 2 or sorted_tipos[1][1] == 0:
        return 'none'
    return sorted_tipos[1][0]

# Usamos TIPOS_SIN_NORMAL para que normal no tape a todos los demás
print("\n[3.2] Calculando tipo dominante (sin contar 'normal')...")
data_frame['tipo_dominante'] = data_frame.apply(
    lambda row: tipo_dominante(row, TIPOS_SIN_NORMAL), axis=1
)
data_frame['segundo_tipo'] = data_frame.apply(
    lambda row: segundo_tipo_dominante(row, TIPOS_SIN_NORMAL), axis=1
)

print("\n  Distribución de tipos dominantes (sin normal):")
print(data_frame['tipo_dominante'].value_counts().to_string())

# Verificación P1
print("\n[3.4] INTERPRETACIÓN DE CLUSTERS - PREGUNTA 1 (con verificación):")
interpretaciones_p1 = {}
for cluster in range(18):
    subset = data_frame[data_frame['cluster_p1'] == cluster]
    tipos_freq = subset['tipo_dominante'].value_counts()
    top_tipo = tipos_freq.index[0] if len(tipos_freq) > 0 else 'N/A'
    pct = tipos_freq.iloc[0] / len(subset) * 100
    interpretaciones_p1[cluster] = top_tipo
    print(f"  Cluster {cluster:2d} ({len(subset):3d} Pokémon) → Tipo dominante: {top_tipo:10s} ({pct:.1f}%)")
    print(f"           Distribución: {dict(tipos_freq.head(3))}")

# Verificación P2
print("\n[3.5] INTERPRETACIÓN DE CLUSTERS - PREGUNTA 2 (con verificación):")
interpretaciones_p2 = {}
for cluster in range(18):
    subset = data_frame[data_frame['cluster_p2'] == cluster]
    tipos_freq = subset['tipo_dominante'].value_counts()
    top_tipo = tipos_freq.index[0] if len(tipos_freq) > 0 else 'N/A'
    pct = tipos_freq.iloc[0] / len(subset) * 100
    interpretaciones_p2[cluster] = top_tipo
    print(f"  Cluster {cluster:2d} ({len(subset):3d} Pokémon) → Tipo dominante: {top_tipo:10s} ({pct:.1f}%)")
    print(f"           Distribución: {dict(tipos_freq.head(3))}")

# Archivos finales
resultado_p1_verificado = data_frame[['pokemon', 'cluster_p1', 'tipo_dominante', 'segundo_tipo']].copy()
resultado_p2_verificado = data_frame[['pokemon', 'cluster_p2', 'tipo_dominante', 'segundo_tipo']].copy()
resultado_p1_verificado.to_csv('resultado_p1_verificado.csv', index=False)
resultado_p2_verificado.to_csv('resultado_p2_verificado.csv', index=False)

data_frame_final = data_frame[['pokemon', 'cluster_p1', 'cluster_p2', 'tipo_dominante', 'segundo_tipo']].copy()
data_frame_final['nombre_cluster_p1'] = data_frame_final['cluster_p1'].map(interpretaciones_p1)
data_frame_final['nombre_cluster_p2'] = data_frame_final['cluster_p2'].map(interpretaciones_p2)
data_frame_final.to_csv('resultado_final_completo.csv', index=False)

print("\n[3.6] RESUMEN FINAL - NOMBRE DE CADA CLUSTER:")
print("\n  Pregunta 1 (TF-IDF general con bigramas):")
for cluster, tipo in interpretaciones_p1.items():
    print(f"  Cluster {cluster}: '{tipo.upper()}'")

print("\n  Pregunta 2 (Vocabulario controlado - 18 tipos):")
for cluster, tipo in interpretaciones_p2.items():
    print(f"  Cluster {cluster}: '{tipo.upper()}'")

print("\n" + "=" * 70)
print("EJECUCIÓN COMPLETADA")
print("=" * 70)
print("\nArchivos generados:")
print("  - resultado_pregunta1.csv")
print("  - resultado_pregunta2.csv")
print("  - resultado_p1_verificado.csv")
print("  - resultado_p2_verificado.csv")
print("  - resultado_final_completo.csv")
